...
function addTranslation(text) {
  ...
  const config = getConfigFromForm();
  if (translations.length > config.maxLines) {
    translations.shift();
  }
...
function renderTranslations() {
  ...
  // Keep only maxLines translations
  while (translations.length > config.maxLines) {
    translations.shift();
  }
...
function calculatePreviewScale() {
  ...
  // Apply transform to scale down the preview (from top-left corner)
